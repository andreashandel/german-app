...
they rescale when you switch to a shorter list.

The range counts **positions among the types you ticked**, always in order of
how common the words are. Tick only Nouns and `1`–`100` gives the hundred most
common nouns; leave every type ticked and `200`–`400` is simply words 200 to 400
of the list. The line under the sliders spells out what is currently selected,
so there is nothing to work out:

> Positions 1–100 of the 242 nouns you have ticked, in order of how common they are.

One consequence worth knowing: because the range is measured after filtering,
the selection is always as wide as the sliders (or as wide as the pool allows).
Ticking and unticking types changes *which* words fall in the range, never how
many.
...
The app's mark is a Brezel. In the page — the header and the streak chip — it is
simply the 🥨 character, so it renders in whatever style the device already
uses and the two always match each other.

Home-screen and tab icons have to be real images, so those are generated by
rendering the same emoji from the system colour-emoji font onto the brand green:

```bash
python tools/make-icons.py
```

That rewrites every file in `icons/`. It needs Pillow and a colour emoji font,
and is the only part of the project that touches Python — the app itself is
unaffected.

Worth knowing: the committed icons were rendered on Windows, so the home-screen
icon carries Microsoft's pretzel while the in-page mark on an iPad is Apple's.
Both are pretzels and both look right; they are just not the same drawing. Run
the script on a Mac to make the icon match iOS.
...
sites not visited in about seven days, which would wipe your progress and your
streak. Adding it to the home screen exempts the app from that, and is the only
thing protecting your history — there is no backup or export. If you clear the
browser's data, it is gone.
...
tools/make-icons.py      regenerates icons/ from the pretzel emoji
sw.js                    offline cache
```
